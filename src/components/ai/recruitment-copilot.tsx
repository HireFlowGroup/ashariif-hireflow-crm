...
import {
  CopilotSourcesPanel,
  type CopilotSourceEntry,
} from "@/components/ai/copilot-sources-panel";
import {
  createConversation,
...
type RecruitmentCopilotProps = {
  isConfigured: boolean;
  className?: string;
};

const CONVERSATIONS_QUERY_KEY = ["copilot-conversations"] as const;
...
export function RecruitmentCopilot({ isConfigured, className }: RecruitmentCopilotProps) {
  ...
  const [sourceEvents, setSourceEvents] = useState<CopilotSourceEntry[]>([]);

  const appendSourceEvent = useCallback((event: ChatStreamToolEvent) => {
  ...
  const sendUserMessage = useCallback(
    async (rawText: string) => {
      ...
      if (!isConfigured) {
        setErrorMessage(
          "OpenAI is niet geconfigureerd. Stel de API key in via Settings → Providers.",
        );
        return;
      }
  ...
  return (
    <div
      className={cn(
        "flex h-[min(780px,calc(100vh-12rem))] overflow-hidden rounded-2xl border bg-card shadow-sm",
        className,
      )}
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <AiComposer
          ...
          placeholder="Stel een recruitment-vraag… (bijv. Welke leads moet ik vandaag bellen?)"
        />
      </div>
      <CopilotSourcesPanel events={sourceEvents} isStreaming={isStreaming} />
    </div>
  );
}
